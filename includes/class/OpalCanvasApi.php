...
class OpalPatient {
	...
	private $CANVAS_API_URL;
	private $CANVAS_API_TOKEN;

	public function __construct(){
		$this->CANVAS_API_URL   = get_option('opal_api_url');
		$this->CANVAS_API_TOKEN = get_option('opal_api_token');
	}
	...
	public function canvas_create( $data ) {
		...
		if ( $status != 400 ) {
			...
			$str2 = str_replace( $this->CANVAS_API_URL . '/Patient/', '', $str[0] );
...
class OpalAppointment {
	...
	public function create_appointment( $appointment_data ) {
		...
		$response = $client->request( 'POST',
			$this->CANVAS_API_URL . '/Appointment/',
			[
				'body'    => $body,
				'headers' => [
					'Authorization' => 'Bearer ' . $this->CANVAS_API_TOKEN,
		...
		if ( $status == 201 || status == 200) {
			$str  = $response->getHeader( 'Location' );
			$str2 = str_replace( $this->CANVAS_API_URL . '/Appointment/', '', $str[0] );
			$str3 = str_replace( '/_history/1', '', $str2 );
	...
	public function get_appointment( $appointment_id ){
		...
		$response = $client->request( 'GET',
			$this->CANVAS_API_URL . '/Appointment/'.$appointment_id,
			[
				'headers' => [
					'Authorization' => 'Bearer ' . $this->CANVAS_API_TOKEN,
					'accept'        => 'application/json',
...
class OpalFormatQuestionnaire {
	...
	public function send_questionnaire( $patient_id ) {
		...
		$response = $client->request( 'POST',
			$this->CANVAS_API_URL . '/QuestionnaireResponse/',
			[
				'body'    => json_encode( $data ),
				'headers' => [
					'Authorization' => 'Bearer ' . $this->CANVAS_API_TOKEN,
		...
		if ( $status != 400 ) {
			$str  = $response->getHeader( 'Location' );
			$str2 = str_replace( $this->CANVAS_API_URL . '/QuestionnaireResponse',
				'',
				$str[0] );
...
class OpalConsentPatient {
	...
	public function create_consent( $data ) {
		...
		$response = $client->request( 'POST', $this->CANVAS_API_URL . '/Consent/',
			[
				'body'    => $body,
				'headers' => [
					'Authorization' => 'Bearer ' . $this->CANVAS_API_TOKEN,
